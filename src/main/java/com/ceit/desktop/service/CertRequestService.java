...
//                        fileStream.close();
//
//                        base64String = IconToBase64String(file);    //生成base64string
...
//        return new Result("success", 100, "传输请求错误，上传失败。");
        String name = request.getParameter("sw_name");
        String desc = request.getParameter("sw_desc");
        String type = request.getParameter("sw_type");
        String sw_public = request.getParameter("sw_public");
        String org_id =  request.getParameter("sw_organization");
        String version = request.getParameter("sw_version");
        if (name == null || name.length() == 0) {
            return new Result("没有软件名称，上传失败", 500, null);
        }

        Map<String,String> fileMap = SoftwareUtil.Upload(request, "software");
        if(fileMap == null)
...
//        }
        for (Map.Entry<String,String> item: fileMap.entrySet()) {
            ...
            String md5Hash = SoftwareUtil.GetFileMD5(f1);
            WebToControlClient webToControlClient = new WebToControlClient();
...
//                sql = "update meeting_file set meeting_id=?, name=?, `size`=?, hash=?, `path`=?, upload_time=? where id="+fileId;
            int count = simpleJDBC.update(sql,
                    name,desc,type,"1",org_id,version, LocalDateTime.now(), sizeRecalculation(f1.length()), savename, md5Hash,"1", IconToBase64String(f1));
            if (count <= 0) {
                f1.delete();
                return new Result("上传失败:写数据库错误", 100, filename);
            }
        }
